...
class UserControllerTests {
    ...
    public void initialization() throws Exception {
    	...
    	userRequest.setUsername("Tarun2");
    ...
    public void testGetUserWithUsernameAndIdApis() throws Exception {
    	...
    	mockMvc.perform(MockMvcRequestBuilders.get("/api/user/{username}", "Tarun2")
    		.accept(MediaType.APPLICATION_JSON).header("Authorization", request.getParameter("Authorization")))
    		.andExpect(status().isOk());

    	mockMvc.perform(
    		MockMvcRequestBuilders.get("/api/user/{username}", "Tarun2").accept(MediaType.APPLICATION_JSON))
    		.andExpect(status().isUnauthorized());

    	mockMvc.perform(MockMvcRequestBuilders.get("/api/user/{username}", "Tarun3")
    		.accept(MediaType.APPLICATION_JSON).header("Authorization", request.getParameter("Authorization")))
